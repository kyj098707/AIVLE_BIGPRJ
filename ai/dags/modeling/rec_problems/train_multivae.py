...
from modeling.rec_problems.model import MultiVAE, loss_function_vae
...
def train_multivae():
    ...
    log_dir_name = str(datetime.datetime.now())[0:10] + '_multivae'
    ...
    with open(os.path.join(log_dir, 'best_multivae_' + args.save), 'rb') as f:
        model.load_state_dict(torch.load(f))

    with open(os.path.join(args.save_dir, 'best_multivae_' + args.save), 'wb') as f:
        torch.save(model.state_dict(), f)
    ...
    with open(os.path.join(log_dir, "update_count_multivae.txt"), "w", encoding='utf-8') as f:
